            ...
            ADALProxy.getOrCreateCtx(authority, validateAuthority).then(function (context) {
                ...
                if (isPhone) {
                    ...
                    try {
                        if (typeof userIdentifier !== 'undefined') {
                            if (typeof extraQueryParameters === 'undefined') {
                                ...
                                context.acquireTokenAndContinue(resourceUrl, clientId, redirectUrl, userIdentifier, extraQueryParameters, claims, function (res) {
                                    handleAuthResult(win, fail, res);
                                });
                            }
                        } else {
                    ...
                    if (context.useCorporateNetwork) {
                        // Try to SSO first
                        context.acquireTokenAsync(resourceUrl, clientId, Windows.Security.Authentication.Web.WebAuthenticationBroker.getCurrentApplicationCallbackUri(), Microsoft.IdentityModel.Clients.ActiveDirectory.PromptBehavior.never, userIdentifier, extraQueryParameters, claims).then(function (res) {
                            handleAuthResult(win, function() {
                                context.acquireTokenAsync(resourceUrl, clientId, Windows.Security.Authentication.Web.WebAuthenticationBroker.getCurrentApplicationCallbackUri(), Microsoft.IdentityModel.Clients.ActiveDirectory.PromptBehavior.always, userIdentifier, extraQueryParameters, claims).then(function (res) {
                                    handleAuthResult(win, fail, res);
                                }, fail);
                            }, res);
                        }, fail);
                    } else {
                        context.acquireTokenAsync(resourceUrl, clientId, redirectUrl, Microsoft.IdentityModel.Clients.ActiveDirectory.PromptBehavior.always, userIdentifier, extraQueryParameters, claims).then(function (res) {
                            handleAuthResult(win, fail, res);
                        }, fail);
                    }
                }
